...
<div class="min-h-screen pb-6">
  ...
  <div class="bg-white border-b border-gray-100 flex sticky top-14 z-40">
    <button onclick="showTab('coupons')" id="tab_coupons" class="flex-1 py-3.5 text-sm font-semibold border-b-2 border-green-500 text-green-600">쿠폰</button>
    <button onclick="showTab('qr')" id="tab_qr" class="flex-1 py-3.5 text-sm font-semibold border-b-2 border-transparent text-gray-400">QR</button>
    <button onclick="showTab('usage')" id="tab_usage" class="flex-1 py-3.5 text-sm font-semibold border-b-2 border-transparent text-gray-400">사용내역</button>
    <button onclick="showTab('settlement')" id="tab_settlement" class="flex-1 py-3.5 text-sm font-semibold border-b-2 border-transparent text-gray-400">정산</button>
  </div>
  <div class="p-4">
...
<!-- 쿠폰 추가 모달 -->
<div id="addCouponModal" class="modal-bg hidden">
  <div class="modal" onclick="event.stopPropagation()">
    <div class="modal-handle"></div>
    <div class="modal-title">쿠폰 추가</div>
    <div class="space-y-3 mb-5">
      ...
      <div class="flex gap-3">
        <div style="flex:1">
          <label class="text-xs text-gray-500 mb-1 block">정가 (원)</label>
          <input id="c_orig" type="number" placeholder="10000" class="input" min="1000">
        </div>
        <div style="flex:1">
          <label class="text-xs text-gray-500 mb-1 block">판매가 (원)</label>
          <input id="c_disc" type="number" placeholder="9000" class="input" min="100">
        </div>
      </div>
      <div>
        <label class="text-xs text-gray-500 mb-1.5 block">이용 횟수 (1~10회)</label>
        <select id="c_count" class="input">
          <option value="1">1회</option><option value="2">2회</option><option value="3">3회</option>
          <option value="4">4회</option><option value="5">5회</option><option value="6">6회</option>
          <option value="7">7회</option><option value="8">8회</option><option value="9">9회</option>
          <option value="10">10회</option>
        </select>
      </div>
      <div>
        <label class="text-xs text-gray-500 mb-1 block">판매 수량 (비워두면 무제한)</label>
        <input id="c_stock" type="number" placeholder="무제한" class="input" min="1">
      </div>
    </div>
    <div class="flex gap-3">
      <button onclick="closeModal('addCouponModal')" class="btn btn-gray" style="flex:1">취소</button>
...
function showTab(tab) {
  ['coupons','qr','usage','settlement'].forEach(t=>{
    document.getElementById('tab_'+t).className='flex-1 py-3.5 text-sm font-semibold border-b-2 '+(t===tab?'border-green-500 text-green-600':'border-transparent text-gray-400');
...
async function loadCoupons() {
  ...
  el.innerHTML='<div class="text-center py-10"><i class="fas fa-spinner fa-spin text-green-400 text-2xl"></i></div>';
  try {
    const r=await API.get('/coupons/owner/stations/'+stationId+'/coupons'); const coupons=r.coupons||[];
    el.innerHTML='<button onclick="openModal(\'addCouponModal\')" class="btn btn-primary mb-4"><i class="fas fa-plus mr-2"></i>쿠폰 추가</button>'
    +(coupons.length?coupons.map(c=>'<div class="card mb-3"><div class="flex justify-between items-start mb-2"><div class="flex-1 min-w-0"><h3 class="font-semibold text-gray-800 truncate">'+c.title+'</h3>'+(c.description?'<p class="text-xs text-gray-400 mt-0.5">'+c.description+'</p>':'')+'</div><label class="relative inline-flex items-center cursor-pointer ml-3 flex-shrink-0"><input type="checkbox" '+(c.is_active?'checked':'')+' onchange="toggleCoupon('+c.id+',this.checked)" class="sr-only peer"><div class="w-11 h-6 bg-gray-200 peer-checked:bg-green-500 rounded-full peer transition-all after:content-[\'\'] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-5"></div></label></div><div class="flex items-baseline gap-2 mb-1"><span class="text-lg font-bold text-green-600">'+formatPrice(c.discount_price)+'</span><span class="text-sm text-gray-300 line-through">'+formatPrice(c.original_price)+'</span><span class="badge badge-red">'+Math.round((1-c.discount_price/c.original_price)*100)+'%</span></div><p class="text-xs text-gray-400">'+c.wash_count+'회권 · 판매 '+(c.active_purchases||0)+'건</p></div>').join('')
    :'<div class="card text-center py-12 text-gray-400"><i class="fas fa-ticket-alt text-4xl mb-3 text-gray-200"></i><p>등록된 쿠폰이 없습니다</p></div>');
...
async function saveCoupon() {
  ...
  if(!document.getElementById('c_title').value)return showToast('쿠폰명을 입력해주세요.','error');
...
async function loadQR() {
  ...
  try {
    ...
    el.innerHTML='<div class="card text-center"><h3 class="font-bold text-gray-800 mb-1">'+r.station_name+'</h3><p class="text-xs text-gray-400 mb-5">고객이 세차 후 이 QR 코드를 스캔합니다</p><canvas id="qrCanvas" class="mx-auto mb-4" style="border-radius:12px"></canvas><p class="text-xs text-gray-200 break-all mb-4">'+r.qr_code+'</p><button onclick="downloadQR()" class="btn btn-outline"><i class="fas fa-download mr-2"></i>QR 이미지 저장</button></div>';
...
async function loadUsages() {
  ...
  try {
    ...
    el.innerHTML='<p class="text-xs text-gray-400 mb-3">총 '+r.total+'건</p>'+(usages.length?usages.map(u=>'<div class="card mb-2"><div class="flex justify-between items-center"><div class="flex-1 min-w-0"><p class="text-sm font-medium text-gray-800 truncate">'+u.coupon_title+'</p><p class="text-xs text-gray-400 mt-0.5">'+u.user_name+' · '+formatDateTime(u.used_at)+'</p></div><div class="text-right ml-2 flex-shrink-0"><p class="font-semibold text-gray-800">'+formatPrice(u.unit_price)+'</p><span class="badge '+(u.settled?'badge-gray':'badge-amber')+'">'+(u.settled?'정산완료':'대기')+'</span></div></div></div>').join(''):'<div class="card text-center py-10 text-gray-400">사용 내역이 없습니다</div>');
  } catch { el.innerHTML='<div class="card text-center py-8 text-red-400">불러올 수 없습니다</div>'; }
}
async function loadSettlements() {
  ...
  try {
    ...
    el.innerHTML='<div class="card mb-4" style="background:linear-gradient(135deg,#ecfdf5,#d1fae5);border:1px solid #a7f3d0"><p class="text-xs text-gray-500 mb-1">정산 대기 금액</p><p class="text-3xl font-bold text-green-600">'+formatPrice(r.pending_amount)+'</p><p class="text-xs text-gray-400 mt-1.5">수수료 15% 차감 후 익일 지급</p></div>'
    +'<h3 class="section-title">정산 내역</h3>'+(list.length?list.map(s=>'<div class="card mb-2"><div class="flex justify-between items-start"><div><p class="font-medium text-gray-800">'+s.settlement_date+'</p><p class="text-xs text-gray-400 mt-0.5">'+s.usage_count+'건 · 수수료 '+formatPrice(s.platform_fee)+'</p></div><div class="text-right"><p class="font-bold text-gray-800">'+formatPrice(s.net_amount)+'</p><span class="badge '+(s.status==='completed'?'badge-green':'badge-amber')+'">'+(s.status==='completed'?'지급완료':'처리중')+'</span></div></div></div>').join(''):'<div class="card text-center py-10 text-gray-400">정산 내역이 없습니다</div>');
  } catch { el.innerHTML='<div class="card text-center py-8 text-red-400">불러올 수 없습니다</div>'; }
}
</script>
`)
